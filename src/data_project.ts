...
class DataProjectController implements Controller {
  ...
  async post(request: Request, res: Response) {
    try {
        const { name, description, image, contact, ressources }  = await request.body;
        if (!name||!description||!image||!contact||!ressources) {
            res.status(401).send("One or more attribute is undefined")
        }else{
            const result = await pg_client.query("INSERT INTO data_project (name, description, image, contact, ressources) VALUES ($1, $2, $3, $4, $5)", [name, description, image, contact, ressources]);
  ...
  async get_data_project(req: Request, res: Response) {
    try {  
      const result = await pg_client.query("SELECT * FROM data_project where id='"+req.params.id+"';");
  ...
  async delete(req: Request, res: Response) {
    try {  
      const result = await pg_client.query("DELETE FROM data_project where id='"+req.params.id+"';");
      if(result.rowCount>0){
